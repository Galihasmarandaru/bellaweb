...
export function AccomodationList() {
  const {
    data: accomodations,
    isLoading: isAccomodationsLoading,
    isError: isAccomodationsError,
  } = useAccomodations();
  const {
    data: categories,
        ...
        {filteredAccomodations?.length === 0 ? (
          <div className="col-span-full flex min-h-[50vh] items-center justify-center">
            <p className="text-gray-500 text-center text-lg">
              Tidak ada data ditemukan, buat minimal 1 Bella!
            </p>
          </div>
        ) : (
            ...
            <Card key={item.id} className="flex flex-col">
              <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 gap-4">
                <div className="flex flex-col gap-1.5 overflow-hidden">
                  <CardTitle className="truncate" title={item.nama_penginapan}>
                ...
                <Button
                  ...
                  onClick={() =>
                    updateMutation.mutate({
                      id: item.id,
                      payload: { is_favourite: !item.is_favourite },
                    })
                  }
                  disabled={updateMutation.isPending}
                >
                  <Heart
                    className={`h-5 w-5 ${item.is_favourite ? 'fill-red-500 text-red-500' : 'text-slate-400'}`}
                  />
                </Button>
              </CardHeader>
              ...
              <CardFooter className="mt-auto flex justify-between gap-2 pt-4">
                {item.image_url ? (
                  <Dialog>
                    <DialogTrigger
                      render={
                        <Button className="bg-[#1b3b2c] text-white hover:bg-[#1b3b2c]/90 shadow-sm">
                          Lihat Foto
                        </Button>
                      }
                    />
